...
- **Manifest detection:** Docgen reads build manifests to detect the language,
  version, and frameworks, and feeds that context to the model so it uses the
  correct terminology. Supported manifests include `package.json` (Node/JS/TS),
  `pyproject.toml` (Python), `Cargo.toml` (Rust), `go.mod` (Go),
  `pubspec.yaml` (Dart/Flutter), `build.gradle` / `build.gradle.kts` (JVM:
  Java/Kotlin/Groovy), `pom.xml` (Maven: Java/Kotlin/Scala), `composer.json`
  (PHP), `Gemfile` (Ruby), `*.csproj` (C#/F#/.NET), `Package.swift` (Swift),
  and `mix.exs` (Elixir).
- **Extension-based detection:** for any language without a manifest (or as a
  cross-check), Docgen infers the primary language from source-file extensions
  across 40+ languages — Python, JavaScript/TypeScript, Go, Rust, Java, Kotlin,
  Scala, Swift, Ruby, PHP, C/C++, C#, F#, Dart, Elixir, Erlang, Haskell, Lua,
  Julia, and more — so polyglot projects (e.g. a Flutter app with a Node
  backend) are labeled correctly.
